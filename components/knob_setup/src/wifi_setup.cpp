...
static char s_ap_name[33] = "knob";

static void configure_ap(wifi_config_t *cfg) {
  *cfg = {};
  strncpy(reinterpret_cast<char *>(cfg->ap.ssid), s_ap_name,
          sizeof(cfg->ap.ssid) - 1);
  cfg->ap.ssid_len = strlen(s_ap_name);
  cfg->ap.channel = 1;
...
static esp_err_t handle_connect(httpd_req_t *req) {
  ...
  httpd_resp_set_type(req, "application/json");
  httpd_resp_sendstr(req, "{\"ok\":false,\"error\":\"Could not connect to "
                          "WiFi. Check your password.\"}");
  return ESP_OK;
}
...
bool wifi_setup_has_credentials() {
  ...
  nvs_handle_t h;
  if (nvs_open("radio", NVS_READONLY, &h) != ESP_OK)
    return false;
  char ssid[33] = {};
...
void wifi_setup_start(const char *ap_name) {
  if (ap_name && ap_name[0]) {
    strncpy(s_ap_name, ap_name, sizeof(s_ap_name) - 1);
  ...
  ESP_LOGI(TAG, "AP started: SSID=%s, IP=192.168.4.1", s_ap_name);
