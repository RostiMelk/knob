...
static void ws_event_handler(void *arg, esp_event_base_t base, int32_t id,
  ...
  case WEBSOCKET_EVENT_DATA: {
    if (event->op_code == 0x01 && event->data_len > 0) {
      ...
      char *json = static_cast<char *>(
          heap_caps_malloc(event->data_len + 1, MALLOC_CAP_SPIRAM));
      if (!json) {
...
static bool send_session_update() {
  ...
  int ret = esp_websocket_client_send_text(s_ws_client, buf, len,
                                           pdMS_TO_TICKS(5000));
  heap_caps_free(buf);
...
static bool send_audio_frame(const int16_t *samples, size_t num_samples) {
  ...
  char *frame =
      static_cast<char *>(heap_caps_malloc(frame_size, MALLOC_CAP_SPIRAM));
  if (!frame)
    return false;
...
static void voice_task_fn(void *) {
  ...
  EventBits_t bits = xEventGroupWaitBits(
      s_events, BIT_WS_CONNECTED | BIT_WS_ERROR | BIT_STOP_REQUESTED, pdTRUE,
  ...
  bits = xEventGroupWaitBits(
      s_events, BIT_SESSION_READY | BIT_WS_ERROR | BIT_STOP_REQUESTED, pdTRUE,
...
void voice_task_start() {
  ...
  BaseType_t ret =
      xTaskCreatePinnedToCore(voice_task_fn, "voice", VOICE_TASK_STACK, nullptr,
                              VOICE_TASK_PRIO, &s_task_handle, VOICE_TASK_CORE);
